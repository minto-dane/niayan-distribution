...
Debian Liveの標準`apparmor.service`には
`ConditionPathExists=!/run/live/overlay/work`があり、対象Liveではprofileを読み込まない。
これを取り除かず、`niaos-hostctl-confinement.service`でNiaのprofileだけを読み込む。
このoneshotはMAC_ADMINだけのcapability、read-onlyなsystem領域、private tmp、
namespace作成制限、メモリ128 MiB/16 tasks/30秒上限で動作する。
標準AppArmorサービスより後、sysinit.targetより前に置く。停止時にprofileを解除して
保護を失わせるExecStopは持たない。完全置換時の有効化はNiaの設定効果へ接続する。
...
加えて`nia-hostctl`がenforceで存在することを必須にする。
...
`test_installed.py`は既存のUEFI導入試験qcow2をread-onlyのbackingとする新しい
差分ディスクだけに設定を置く。Secure Bootで一度起動して設定とunitを保存し、
正常な電源オフ後の2回目の起動で自動適用・全30検査・元ファイルhash・service状態を確認する。
元ディスクの前後hashも比較する。Nia package transaction経由での導入ではないため、
この成功をパッケージ管理置換の成功に数えない。

```sh
python3 hardening/test_installed.py --base-disk /fixtures/installed.qcow2 \
  --output /new-output/hardening-installed
```

BIOS・UEFI・Secure BootのLive 3試験と、設定保存後のSecure Boot再起動試験が成功した。
失敗した初期検査と修正理由を含む[実行記録](../evidence/hardening/desktop-01/README.ja.md)を保存する。
