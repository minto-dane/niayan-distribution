...
`root_bank.py`は非公開世代を実ファイルへ展開するroot-session用の共有内部処理である。
既存catalogやaccepted rootを更新せず、導入済みpackageの第二DBも作らない。
この実装だけでは本番の世代認可・供給検査・公開・起動に接続していない。

## 保護された設定と接続

旧listener/RPCは廃止した。Bankはroot sessionが所有するlibraryであり、単独で要求を受け付けない。
CLIは明示した`--config`と`--provision-bank`を伴う初期化専用である。
配備/更新条件は[service-deployment.ja.md](service-deployment.ja.md)を参照。
...
通常起動は両方を必須とし、欠損を自動初期化しない。bank.lockの排他flockはBankの全生存期間保持する。
reservationはcore所有0600・単一link・長さ0の既存CAS lockで、そのinodeをBank生成時に固定する。
lockの再作成や別inodeへの移行は、処理を停止して行う独立した復旧操作を要する。

## 依頼と実行

Bank.prepareはcontrollerから検証対象request、読取専用archive FD、O_RDWRのCAS予約FDを受ける。
通信の本人確認とFD受け渡しは[root session](root-session.ja.md)の責務である。
requestのversion、stage、generation/root_manifest/archive、size、entries、deadline_msを検査する。

受け取った予約のinode・所有者・mode・長さ・access modeを検査し、排他flockを保持する。
同じopen file descriptionをworkerまで引き継ぎ、Bank側からLOCK_UNしない。
...
`make native-bank-check WORKER_TEST_BASE=<使い捨てVMの専用mount>`は共有Bankと実worker、呼出し元の予約維持、別予約の拒否、既存stage拒否、再起動後の記録を確認する。
共有Bankのintent永続化後に所有するprocessを停止・強制終了し、成功を推定しないことも確認する。
通常のhost実行用unit testではなく、VMと外側cgroupの制限内で逐次実行する。

## 固定したrootの現在照合

`Bank.verify(request, archive_fd, lease_fd)`にはprepareと同じ種類の二つのFDを渡す。
...
`make native-reinspection-check WORKER_TEST_BASE=<使い捨てVMの専用mount>`で共有Bankと実workerを検査する。
