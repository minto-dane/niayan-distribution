...
def peer_alive(pidfd):
    ...
    if poll.poll(0):raise Rejected('controller-supervisor-exited')


def serve_session(peer):
    descriptors=[];bank=None;frozen=None;device=base=pidfd=-1;attempt=False;readonly=False
    ...
    try:
        ...
        # Linux SO_PEERPIDFD pins the connecting task, including PID reuse.
        pidfd=peer.getsockopt(socket.SOL_SOCKET,77)
        os.set_inheritable(pidfd,False);peer_alive(pidfd)
        ...
        for fd in descriptors:os.close(fd)
        descriptors=[]
        peer_alive(pidfd);peer.settimeout(remaining(deadline))
        ...
        print(canonical({'event':'controller-refusal','error':type(error).__name__,'reason':str(error)[:256]}).decode(),file=sys.stderr,end='')
