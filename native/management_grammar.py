...
"""Adopted command grammar shared by client and privileged request decoder.

Parsing performs no filesystem reads, network access or execution. Operands
remain untrusted selectors; only the native planner resolves package inputs.
"""
...
READ_ONLY_ACTIONS = frozenset({
    'installed-list', 'installed-summary', 'applied-list', 'history', 'files',
    'requisites', 'dependents', 'owners', 'check-content', 'check-size',
    'check-links', 'check-consistency', 'fix-status', 'fix-packages', 'media-fixes',
    'interim-list', 'interim-check', 'interim-locks', 'fetch-task-list',
    'fetch-config', 'fetch-defaults', 'media-filter-list',
})


class UsageError(ValueError):
    def __init__(self, message: str, **values: str) -> None:
...
def check_schedule(value: str) -> None:
    fields = value.split()
    ...
    for field, (low, high) in zip(fields, bounds):
        for item in field.split(','):
            ...
            base, step = cast(tuple[str, str], match.groups(default=''))
...
def parse(command: str, argv: list[str]) -> Request:
    ...
    def require(condition: bool, message: str, **values: str) -> None:
        if not condition:
    ...
    if command == 'installp':
        ...
        if key == 'd':
            ...
            count = sum(key in values for key in 'Lnu')
