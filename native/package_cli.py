...
def parse(command: str, argv: list[str]) -> Request:
    ...
    for option, value in options:
        ...
        if key + ':' in SPECS[command]:
            if key in values:
                raise UsageError('repeated -' + key)
            if not value or (value.startswith('-') and key != 'I' and not (key == 'f' and value == '-')):
                raise UsageError('missing or unsupported value for -' + key)
            values[key] = value
        else:
            flags.add(key)
    ...
    def only(allowed: str):
        require((flags | values.keys()) <= set(allowed), 'option is not valid for this action')

    def one(choices: str) -> str:
        selected = flags & set(choices)
        require(len(selected) == 1, 'specify exactly one action from -' + ', -'.join(choices))
    ...
    if command == 'installp':
        ...
        if not actions:
            actions = {'a'}
        require(len(actions) == 1 or actions == {'a', 'c'}, 'conflicting installp actions')
        ...
        if key in ('a', 'ac', 'c', 'r', 'u'):
            ...
            require(bool(operands) != ('f' in values), 'specify operands or -f LIST')
            require('all' not in operands or operands == ['all'], 'all cannot be combined with names')
            if key in ('r', 'u'):
                require('all' not in operands, 'name packages explicitly for reject/remove')
            if 'a' in key:
                require('d' in values, 'Nia requires an explicit -d SOURCE for apply')
            preview = 'p' in flags
        elif key in ('l', 'L'):
            only(key + 'dq')
            require('d' in values and not operands, 'media listing requires -d SOURCE and no operands')
        elif key == 'C':
            only('C')
            require(not operands, 'recovery covers the interrupted transaction; no operands allowed')
        else:
            only('s')
            require('all' not in operands or operands == ['all'], 'all cannot be combined with names')
    elif command == 'lslpp':
        ...
        key = one('cflv')
        only(key + 'm')
        require(values.get('m', '1') in ('1', '2', '3'), '-m must be 1, 2 or 3')
        require(key != 'v' or len(operands) <= 1, '-v does not accept file operands')
        action = {'c': 'check-content', 'f': 'check-size', 'l': 'check-links',
                  'v': 'check-consistency'}[key]
    elif command == 'install_all_updates':
        require('d' in values and not operands, 'specify -d SOURCE, without package operands')
        action = 'update-installed-commit' if 'c' in flags else 'update-installed'
        preview = 'p' in flags
    elif command == 'instfix':
        key = one('ipT')
        require(not operands, 'use -k for fix keywords')
        if key == 'i':
            only('icqvFk')
            action = 'fix-status'
        else:
            only(key + ('kd' if key == 'p' else 'd'))
            require('d' in values, 'specify -d SOURCE')
            require(key != 'p' or 'k' in values, '-p requires -k FIX')
            action = 'fix-packages' if key == 'p' else 'media-fixes'
        if 'k' in values:
            require(bool(values['k'].split()), 'empty fix keyword list')
    elif command == 'inutoc':
        require(len(operands) <= 1, 'inutoc accepts at most one directory')
        if not operands:
            operands = ['/usr/sys/inst.images']
        action = 'index-media'
    elif command == 'geninstall':
        require(not {'u', 'L'} <= flags, 'conflicting geninstall actions')
        if 'L' in flags:
            only('Ld')
            ...
            require(not remainder, '-I accepts only installp option flags')
            args = [option for option, _ in nested]
            require('-a' in args or '-c' not in args, 'geninstall -I must select apply, not commit alone')
            if 'p' in flags:
                args.append('-p')
        ...
        require(all(re.fullmatch('[0-9]{1,20}', item) for item in operands), 'task IDs must be numeric')
        if flags & set('lcDud'):
            key = one('lcDud')
            only(key)
            require(not attributes, 'attribute edits for this SUMA action are not supported')
            require(key == 'l' or len(operands) == (1 if key in 'ud' else 0), 'incorrect task ID count')
            action = {'l': 'fetch-task-list', 'c': 'fetch-config', 'D': 'fetch-defaults',
                      'u': 'fetch-task-unschedule', 'd': 'fetch-task-delete'}[key]
        else:
            require(len(operands) <= 1, 'at most one task ID')
            require(bool(flags & set('xw')) or 's' in values, 'specify -x, -w or -s')
            require('s' not in values or not flags, '-s cannot be combined with -x or -w')
            if 's' in values:
                check_schedule(values['s'])
            require(attributes.get('RqType', 'Latest') == 'Latest', 'Foreign TL/SP/ML/PTF are not Nia release identifiers')
            require(attributes.get('Action', 'Download') in ('Download', 'Preview', 'Metadata'),
                    'unsupported SUMA Action')
            if 'DLTarget' in attributes:
                require(attributes['DLTarget'].startswith('/'), 'DLTarget must be absolute')
        ...
        if key == 'd':
            ...
            require(not operands and (count == 1 if key == 'r' else count <= 1),
                    'specify one selector for remove, at most one for list/check')
            require('n' not in values or bool(re.fullmatch(r'[1-9][0-9]{0,19}', values['n'])),
                    'interim number must be a positive integer')
            action = {'l': 'interim-list', 'c': 'interim-check', 'r': 'interim-remove'}[key]
        elif key == 'P':
            only('P')
            require(len(operands) <= 1, 'at most one package for lock display')
            action = 'interim-locks'
        else:
            only('epq')
            require('e' in values and not operands, 'specify -e PACKAGE_FILE')
            action = 'interim-apply'
        require('v' not in values or values['v'] in ('1', '2', '3'), 'verbosity must be 1, 2 or 3')
        preview = 'p' in flags
    else:  # lppmgr
        require('d' in values and not operands, 'specify -d SOURCE, without operands')
        require(not ('r' in flags and 'm' in values), '-r and -m are mutually exclusive')
...
def main(command: str, argv: list[str]) -> int:
    if command not in HELP:
        print('Invoke as: ' + ', '.join(HELP), file=sys.stderr)
        return 2
    if argv == ['--help']:
        print(HELP[command])
    ...
    print(f'{command}: {request.action}: native package service is not connected; '
          'no operation or preview was performed.', file=sys.stderr)
    return 1
