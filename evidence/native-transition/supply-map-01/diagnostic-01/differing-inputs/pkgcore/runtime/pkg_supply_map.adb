...
package body Pkg_Supply_Map with SPARK_Mode => Off is
   ...
   procedure Evaluate (Store : in out MC_Store.Store; Target : Context; Items : Sources;
      ...
      Position : Natural := Items'First; Old_Position : Positive := 1; Which : Natural;
   ...
   begin
      ...
      for I in 1 .. C.Package_Count (After) loop
         ...
         if Old_Position > C.Package_Count (Old) or else Old_Item.Original /= Item.Original then
            if Position > Items'Last or else Items (Position).Original /= Item.Original
              or else Items (Position).Control /= Item.Control then Status := Conflict; raise Interrupted; end if;
            Position := Position + 1;
         elsif Old_Item.Control /= Item.Control then Status := Corrupt; raise Interrupted;
         end if;
      end loop;
      if Position <= Items'Last then Status := Conflict; raise Interrupted; end if;
