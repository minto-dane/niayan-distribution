...
w=Path('/home/nia/devbox/niaos/.work/native-generation-transition-01');root=Path('/home/nia/devbox/niaos/nia-os-consent')
...
(w/'workspace-reproducibility.json').write_text(json.dumps(dict(result='identical-binaries',binary_count=18,binaries=binaries,input_copies=copies,elf=elf,source_date_epoch=[None,1788739200],independent_mtime=1788652800,timezones=['UTC system default; TZ unset','Pacific/Honolulu'],jobs=1,first_environment='explicit env -u SOURCE_DATE_EPOCH -u TZ for first build; JOBS=1'),indent=2)+'\n')
