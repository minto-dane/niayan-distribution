...
work=Path('/evidence');vmwork=work/'vm-order-01';vmwork.mkdir(exist_ok=False)
...
uname -a
sudo python3 check_root_order.py --inputs /tmp/nia-root-extract/inputs --worker /tmp/nia-root-extract/worker --target-base /run/nia-root-extract-test --report /tmp/nia-root-extract/result.json
