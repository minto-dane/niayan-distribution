...
work=Path('/evidence');vmwork=work/'vm-test-05';vmwork.mkdir(exist_ok=False)
...
with (vmwork/'qemu.log').open('wb') as log:
 ...
 try:
  ...
  subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223',str(work/'package-input-03.tar'),'builder@127.0.0.1:/home/builder/nia-bank-freeze-05.tar'],check=True,timeout=60)
  script='''set -eu
umask 022
mkdir -m 700 /home/builder/nia-bank-freeze-05
cd /home/builder/nia-bank-freeze-05
tar -xf /home/builder/nia-bank-freeze-05.tar
...
sudo truncate -s 32M /tmp/nia-bank-freeze-05-disposable.ext4
sudo /sbin/mkfs.ext4 -q -F /tmp/nia-bank-freeze-05-disposable.ext4
sudo mount -o loop,nodev,nosuid,noexec /tmp/nia-bank-freeze-05-disposable.ext4 /run/nia-root-extract-test
...
sudo python3 tests/worker/check_root_preparation.py --reinspect --freeze-bank --configured --worker /usr/libexec/niaos/root-extract --driver /home/builder/nia-bank-freeze-05/drivers/run_root_configuration_tests --loader /home/builder/nia-bank-freeze-05/lib/ld-linux-x86-64.so.2 --libraries /home/builder/nia-bank-freeze-05/lib --fixtures /home/builder/nia-bank-freeze-05/fixtures-configured --base /run/nia-root-extract-test --report /home/builder/nia-bank-freeze-05/configured/result.json
sudo python3 tests/worker/check_root_preparation.py --reinspect --freeze-bank --worker /usr/libexec/niaos/root-extract --driver /home/builder/nia-bank-freeze-05/drivers/run_root_archive_tests --loader /home/builder/nia-bank-freeze-05/lib/ld-linux-x86-64.so.2 --libraries /home/builder/nia-bank-freeze-05/lib --fixtures /home/builder/nia-bank-freeze-05/fixtures-plain --base /run/nia-root-extract-test --report /home/builder/nia-bank-freeze-05/plain/result.json
...
'''
  ...
  for name in ['configured','plain','install.log']:
   subprocess.run(['/evidence/scp-wrapper','-r','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-bank-freeze-05/'+name,str(vmwork/name)],check=False,timeout=30)
