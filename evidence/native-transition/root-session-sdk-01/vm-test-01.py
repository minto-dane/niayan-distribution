...
work=Path('/evidence');vmwork=work/'vm-test-01';vmwork.mkdir(exist_ok=False)
...
with (vmwork/'qemu.log').open('wb') as log:
 ...
 try:
  ...
  subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223',str(work/'runtime.tar'),'builder@127.0.0.1:/home/builder/nia-root-session-sdk-01.tar'],check=True,timeout=60)
  script='''set -eu
umask 022
mkdir -m 700 /home/builder/nia-root-session-sdk-01
cd /home/builder/nia-root-session-sdk-01
tar -xf /home/builder/nia-root-session-sdk-01.tar
...
sudo python3 check_bank_device.py --device /dev/vdb1 --report /home/builder/nia-root-session-sdk-01/bootstrap.json
sudo python3 check_root_session_sdk.py --library /home/builder/nia-root-session-sdk-01/root-session.so --report /home/builder/nia-root-session-sdk-01/result.json
'''
  ...
  for name in ['result.json','bootstrap.json','install.log']:
   subprocess.run(['/evidence/scp-wrapper','-r','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-root-session-sdk-01/'+name,str(vmwork/name)],check=False,timeout=30)
