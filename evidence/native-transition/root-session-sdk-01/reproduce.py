...
subprocess.run(['gprbuild','-P','tests.gpr','-j1','run_root_session_tests.adb'],cwd=other,check=True)
subprocess.run([str(original/'build/test-bin/run_root_session_tests')],check=True)
subprocess.run([str(other/'build/test-bin/run_root_session_tests')],check=True)
a=hashlib.sha256((original/'build/test-bin/run_root_session_tests').read_bytes()).hexdigest()
b=hashlib.sha256((other/'build/test-bin/run_root_session_tests').read_bytes()).hexdigest()
assert a==b,(a,b)
