...
with (vmwork/'qemu.log').open('wb') as log:
 ...
 try:
  ...
  subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223',str(work/'runtime-03.tar'),'builder@127.0.0.1:/home/builder/nia-root-session-03.tar'],check=True,timeout=60)
  script='''set -eu
umask 022
mkdir -m 700 /home/builder/nia-root-session-03
cd /home/builder/nia-root-session-03
tar -xf /home/builder/nia-root-session-03.tar
...
sudo python3 -c 'from pathlib import Path; p=Path("/run/prior-session"); print([f.name for f in p.iterdir()]); [print(f.read_text()) for pattern in ("*/result.json","*/worker.json") for f in p.glob(pattern)]' > initial-session-diagnosis.jsonl
sudo umount /run/prior-session
sudo python3 check_bank_device.py --device /dev/vdb1 --report /home/builder/nia-root-session-03/bootstrap.json
sudo python3 check_root_session.py --ending close --report /home/builder/nia-root-session-03/result.json
'''
  ...
  if r.returncode==0:
   ...
   with (vmwork/'reboot.log').open('wb') as out:
    r=subprocess.run(ssh+['sudo python3 /home/builder/nia-root-session-03/check_root_session.py --after-reboot --report /home/builder/nia-root-session-03/result.json'],stdout=out,stderr=subprocess.STDOUT,timeout=180)
  ...
  for name in ['result.json','initial-session-diagnosis.jsonl','bootstrap.json','install.log']:
   subprocess.run(['/evidence/scp-wrapper','-r','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-root-session-03/'+name,str(vmwork/name)],check=False,timeout=30)
