...
with (vmwork/'qemu.log').open('wb') as log:
 ...
 try:
  ...
  subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223',str(work/'runtime-03.tar'),'builder@127.0.0.1:/home/builder/nia-root-session-05.tar'],check=True,timeout=60)
  script='''set -eu
umask 022
mkdir -m 700 /home/builder/nia-root-session-05
cd /home/builder/nia-root-session-05
tar -xf /home/builder/nia-root-session-05.tar
...
sudo python3 check_bank_device.py --device /dev/vdb1 --report /home/builder/nia-root-session-05/bootstrap.json
sudo python3 check_root_session.py --ending expire --report /home/builder/nia-root-session-05/result.json
'''
  ...
  for name in ['result.json','bootstrap.json','install.log']:
   subprocess.run(['/evidence/scp-wrapper','-r','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-root-session-05/'+name,str(vmwork/name)],check=False,timeout=30)
