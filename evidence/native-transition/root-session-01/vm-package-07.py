...
work=Path('/evidence');vmwork=work/'vm-package-07';vmwork.mkdir(exist_ok=False)
...
with (vmwork/'qemu.log').open('wb') as log:
 ...
 try:
  ...
  subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223',str(work/'runtime-07.tar'),'builder@127.0.0.1:/home/builder/nia-root-session-07.tar'],check=True,timeout=60)
  script='''set -eu
umask 022
mkdir -m 700 /home/builder/nia-root-session-07
cd /home/builder/nia-root-session-07
tar -xf /home/builder/nia-root-session-07.tar
...
sudo python3 check_bank_device.py --device /dev/vdb1 --report /home/builder/nia-root-session-07/bootstrap.json
sudo python3 check_root_session.py --ending kill --report /home/builder/nia-root-session-07/result.json
'''
  ...
  with (vmwork/'service-journal.log').open('wb') as out:
   subprocess.run(ssh+['sudo journalctl -u niaos-root-session-seal.service -u niaos-root-session.service -u niaos-root-bank-check.service -u niaos-root-preparation.service -u var-lib-niaos-roots.mount --no-pager -n 150'],stdout=out,stderr=subprocess.STDOUT,timeout=20)
  for name in ['result.json','bootstrap.json','build-a.log','build-b.log','package-sha256.txt','install.log']:
   subprocess.run(['/evidence/scp-wrapper','-r','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-root-session-07/'+name,str(vmwork/name)],check=False,timeout=30)
  (vmwork/'packages').mkdir()
  for pattern in ['*.deb','*.dsc','*.tar.xz','*.buildinfo','*.changes']:
   subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-root-session-07/build-a/'+pattern,str(vmwork/'packages')],check=True,timeout=30)
