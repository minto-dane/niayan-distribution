...
base=Path('/home/nia/devbox/niaos/.work/native-transition-plan-01'); root=Path('/home/nia/devbox/niaos/nia-os-consent')
first=base/'accepted';second=base/'independent-long-path/pkgcore'
def binaries(tree):
...
a=binaries(first);b=binaries(second);assert len(a)==28 and a==b
(base/'reproducibility.json').write_text(json.dumps(dict(result='identical-binaries',binary_count=len(a),first_path='/workspace',second_path='/independent/pkgcore',first_mtimes='preserved checkout mtimes',second_mtime=1788652800,timezones=['UTC','Pacific/Honolulu'],binaries=a),indent=2)+'\n')
rows=[]
...
print('28 identical executables; 28 ELF hardening checks;',len(inputs),'inputs match every copy')
