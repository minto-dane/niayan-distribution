...
base=Path('/evidence/upstream-oracle');base.mkdir(mode=0o700,exist_ok=True)
media=Path('/workspace/tests/fixtures/deb-final-set')
version=subprocess.check_output(['dpkg','--version'],text=True).splitlines()[0]
assert '1.22.22' in version,version
matrix=json.loads((media/'cases.json').read_text());results=[];started=time.monotonic()
for case in matrix:
