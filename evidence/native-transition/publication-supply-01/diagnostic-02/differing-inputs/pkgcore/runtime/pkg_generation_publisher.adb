...
package body Pkg_Generation_Publisher with SPARK_Mode => Off is
   ...
   begin
      ...
      if Status = OK then
         ...
         -- An orphan Prepared log is not an admitted transaction. New work must
         -- pass fresh policy even if an old planning snapshot is fully retained.
         if Recorded then
            Pkg_Recovery_Audit.Inspect (State_Path, Store_Path, Expected_Plan, Audit, Status);
         end if;
      end if;
      if Status = OK then
         Supply_Target := (P.Root_ID, Before, Before_Closure, M.Catalog, M.Catalog_Closure);
         Pkg_Supply_Policy.Load (Store, M.Supply_Policy, Deadline, Supply_Value, Status);
      end if;
      if Status = OK then
         declare Current_Policy : Pkg_Supply_Policy.Snapshot; begin
            Check_Supply_Policy (Current_Policy, Status);
            if Status = OK then
               if Recorded then
                  Pkg_Supply_Policy.Recheck_Recorded (Store, M.Supply_Policy, Supply_Target,
                     Current_Policy.Trusted (1 .. Current_Policy.Count), Current_Policy.Observed_At, Deadline, Status);
               else
                  Pkg_Supply_Policy.Verify_New (Store, M.Supply_Policy, Supply_Target,
                     Current_Policy.Trusted (1 .. Current_Policy.Count), Current_Policy.Observed_At, Deadline, Supply_Until, Status);
               end if;
            end if;
            if Status = OK then Check_Supply_Policy (Current_Policy, Status); end if;
            Supply_Checked := Status = OK;
         end;
      end if;
