...
for directory in sorted((work/'chaos').glob('attempt-*')):
    ...
    length=struct.unpack_from('>H',plan,192)[0];assert plan[194:196]==bytes(2) and plan[196:196+length]==b'generation.next' and len(plan)==196+length+256
    offset=196+length;assert plan[offset+168:offset+200]==sha(descriptor)
