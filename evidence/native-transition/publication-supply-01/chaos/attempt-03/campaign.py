...
TRACE = HERE/'strace'
SYSCALLS = ('write', 'fsync', 'renameat', 'renameat2', 'pread64')
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='nia-publication-base-') as temporary:
        ...
        try:
            ...
            all_lines=baseline['trace'].splitlines()
            replacements=[i for i,line in enumerate(all_lines) if line.startswith(('renameat(', 'renameat2('))
                          and str(tree/'state') in line and '"root.state"' in line]
            assert len(replacements)==2,replacements
            # MC_FS.Rename synchronizes both parent directory descriptors after
            # replacement. End the publication trace at the second final sync;
            # the laboratory's subsequent native readback is a different API.
            final_syncs=[i for i,line in enumerate(all_lines) if i>replacements[-1]
                         and line.startswith('fsync(') and str(tree/'state') in line]
            assert len(final_syncs)>=2
            lines=all_lines[:final_syncs[1]+1];points=[]
            report['publication_trace_end']=lines[-1]
            report['publication_replacement_syscalls']=[all_lines[i] for i in replacements]
            for index,line in enumerate(lines):
                ...
                elif syscall in ('renameat','renameat2') and '"root.state"' in line: points.append(index)
            ...
            reads=[(i+1,line) for i,line in enumerate(calls('\n'.join(lines),'pread64')) if policy_hash[2:] in line]
            ...
            rename=next(s for s in schedule if s['kind']=='kill' and s['syscall'] in ('renameat','renameat2'))
            for i,address in enumerate((policy_hash,receipt_hash)):
                ...
                with tempfile.TemporaryDirectory(prefix='nia-publication-stacked-') as temp:
                    tree=copy_base(temp);injection=f"{rename['syscall']}:signal=SIGKILL:when={rename['nth']}"
                    value=invoke(tree,tag+'-kill',injection=injection)
                    hit=calls(value['trace'],rename['syscall'])[rename['nth']-1]
