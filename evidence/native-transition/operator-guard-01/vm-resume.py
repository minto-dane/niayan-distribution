...
assert json.loads((lab/'vm/exit.json').read_text())['qemu'] == 0
...
args=['qemu-system-x86_64','-machine','q35','-accel','kvm','-cpu','host','-m','2048','-smp','1',
      '-drive','file='+str(lab/'vm/system.qcow2')+',if=virtio,format=qcow2,discard=unmap',
...
with (work/'qemu.log').open('wb') as log:
    ...
    try:
        ...
        subprocess.run(scp+[str(lab/'check_operator_guard.py'),'builder@127.0.0.1:/home/builder/niayan-monitor/check_operator_guard.py'],check=True,timeout=60)
        with (work/'guest.log').open('wb') as out:
            result=subprocess.run(ssh+['sh','-s'],input=(lab/'guest-resume.sh').read_bytes(),stdout=out,stderr=subprocess.STDOUT,timeout=420)
        print('Guest result',result.returncode,flush=True)
        with (work/'journal.log').open('wb') as out:
            subprocess.run(ssh+['sudo journalctl -u polkit.service --no-pager -n 120'],stdout=out,stderr=subprocess.STDOUT,timeout=15)
        for name in ['build-component.log','build-controller.log','install.log','result.json','packages.tsv','*.deb','*.dsc','*.tar.xz','*.buildinfo','*.changes']:
