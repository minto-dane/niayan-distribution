...
with (vmwork/'qemu.log').open('wb') as log:
 ...
 try:
  ...
  subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223',str(work/'runtime.tar'),'builder@127.0.0.1:/home/builder/nia-bank-device-01.tar'],check=True,timeout=60)
  script='''set -eu
umask 022
mkdir -m 700 /home/builder/nia-bank-device-01
cd /home/builder/nia-bank-device-01
tar -xf /home/builder/nia-bank-device-01.tar
...
sudo python3 check_bank_device.py --device /dev/vdb1 --report /home/builder/nia-bank-device-01/result.json
'''
  ...
  if r.returncode==0:
   ...
   with (vmwork/'reboot.log').open('wb') as out:
    r=subprocess.run(ssh+['sudo python3 /home/builder/nia-bank-device-01/check_bank_device.py --after-reboot --device /dev/vdb1 --report /home/builder/nia-bank-device-01/result.json'],stdout=out,stderr=subprocess.STDOUT,timeout=180)
  ...
  for name in ['result.json','build-a.log','build-b.log','package-sha256.txt','install.log']:
   subprocess.run(['/evidence/scp-wrapper','-r','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-bank-device-01/'+name,str(vmwork/name)],check=False,timeout=30)
  (vmwork/'packages').mkdir()
  for pattern in ['*.deb','*.dsc','*.tar.xz','*.buildinfo','*.changes']:
   subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-bank-device-01/build-a/'+pattern,str(vmwork/'packages')],check=True,timeout=30)
