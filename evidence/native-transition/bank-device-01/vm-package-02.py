...
with (vmwork/'qemu.log').open('wb') as log:
 ...
 try:
  ...
  subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223',str(work/'runtime-02.tar'),'builder@127.0.0.1:/home/builder/nia-bank-device-02.tar'],check=True,timeout=60)
  script='''set -eu
umask 022
mkdir -m 700 /home/builder/nia-bank-device-02
cd /home/builder/nia-bank-device-02
tar -xf /home/builder/nia-bank-device-02.tar
...
cmp build-a/niaos-root-preparation_0.5.0_amd64.deb build-b/niaos-root-preparation_0.5.0_amd64.deb
cmp build-a/niaos-root-preparation-dbgsym_0.5.0_amd64.deb build-b/niaos-root-preparation-dbgsym_0.5.0_amd64.deb
cmp build-a/niaos-root-preparation_0.5.0.dsc build-b/niaos-root-preparation_0.5.0.dsc
cmp build-a/niaos-root-preparation_0.5.0.tar.xz build-b/niaos-root-preparation_0.5.0.tar.xz
sha256sum build-a/*.deb build-a/*.dsc build-a/*.tar.xz > package-sha256.txt
sudo dpkg -i dependencies/*.deb component.deb build-a/niaos-root-preparation_0.5.0_amd64.deb > install.log 2>&1
sudo systemd-analyze verify niaos-root-preparation.service niaos-root-preparation.socket niaos-root-bank-check.service var-lib-niaos-roots.mount
...
sudo python3 check_bank_device.py --device /dev/vdb1 --report /home/builder/nia-bank-device-02/result.json
'''
  ...
  if r.returncode==0:
   ...
   with (vmwork/'reboot.log').open('wb') as out:
    r=subprocess.run(ssh+['sudo python3 /home/builder/nia-bank-device-02/check_bank_device.py --after-reboot --device /dev/vdb1 --report /home/builder/nia-bank-device-02/result.json'],stdout=out,stderr=subprocess.STDOUT,timeout=180)
  with (vmwork/'service-journal.log').open('wb') as out:
   subprocess.run(ssh+['sudo journalctl -u niaos-root-bank-check.service -u niaos-root-preparation.service -u var-lib-niaos-roots.mount --no-pager -n 150'],stdout=out,stderr=subprocess.STDOUT,timeout=20)
  for name in ['result.json','build-a.log','build-b.log','package-sha256.txt','install.log']:
   subprocess.run(['/evidence/scp-wrapper','-r','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-bank-device-02/'+name,str(vmwork/name)],check=False,timeout=30)
  (vmwork/'packages').mkdir()
  for pattern in ['*.deb','*.dsc','*.tar.xz','*.buildinfo','*.changes']:
   subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-bank-device-02/build-a/'+pattern,str(vmwork/'packages')],check=True,timeout=30)
