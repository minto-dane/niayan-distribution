...
root=Path('/home/nia/devbox/niaos/nia-os-consent'); target=Path('/home/nia/devbox/niaos/.work/native-catalog-store-01/proof-input-comparison.json')
...
target.write_text(json.dumps(dict(result='unchanged',scope='Exact existing proof input sets only; new catalog store runtime is outside SPARK',baseline=str(p.relative_to(root)),baseline_sha256=hashlib.sha256(p.read_bytes()).hexdigest(),components=rows),indent=2)+'\n')
