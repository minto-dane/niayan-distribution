...
work=Path('/evidence');vmwork=work/'vm-package-05';vmwork.mkdir(exist_ok=False)
...
with (vmwork/'qemu.log').open('wb') as log:
 ...
 try:
  ...
  subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223',str(work/'package-input-05.tar'),'builder@127.0.0.1:/home/builder/nia-root-reinspection-05.tar'],check=True,timeout=60)
  script='''set -eu
umask 022
mkdir -m 700 /home/builder/nia-root-reinspection-05
cd /home/builder/nia-root-reinspection-05
tar -xf /home/builder/nia-root-reinspection-05.tar
...
sudo truncate -s 32M /home/builder/nia-root-reinspection-05/disposable.ext4
sudo /sbin/mkfs.ext4 -q -F /home/builder/nia-root-reinspection-05/disposable.ext4
sudo mount -o loop,nodev,nosuid,noexec /home/builder/nia-root-reinspection-05/disposable.ext4 /run/nia-root-extract-test
sudo chmod 711 /run/nia-root-extract-test
uname -a
dpkg-query -W libarchive13t64 libsodium23
sha256sum /usr/libexec/niaos/root-extract
sudo python3 tests/worker/check_root_extract.py --worker /usr/libexec/niaos/root-extract --target-base /run/nia-root-extract-test --report /home/builder/nia-root-reinspection-05/extraction.json
sudo python3 tests/worker/check_root_bank.py --worker /usr/libexec/niaos/root-extract --base /run/nia-root-extract-test --report /home/builder/nia-root-reinspection-05/bank.json
sudo python3 tests/worker/check_root_reinspection.py --worker /usr/libexec/niaos/root-extract --base /run/nia-root-extract-test --report /home/builder/nia-root-reinspection-05/result.json
...
'''
  ...
  if r.returncode==0:
   subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-root-reinspection-05/result.json',str(vmwork/'result.json')],check=True,timeout=30)
  for name in ['extraction.json','bank.json','build-a.log','build-b.log','package-sha256.txt','install.log','reproducibility-diagnosis.json']:
   subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-root-reinspection-05/'+name,str(vmwork/name)],check=False,timeout=30)
  if r.returncode==0:
   (vmwork/'packages').mkdir()
   subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-root-reinspection-05/build-a/*.deb','builder@127.0.0.1:/home/builder/nia-root-reinspection-05/build-a/*.dsc','builder@127.0.0.1:/home/builder/nia-root-reinspection-05/build-a/*.tar.xz',str(vmwork/'packages')],check=True,timeout=30)
