...
work=Path('/evidence');vmwork=work/'vm-package-03';vmwork.mkdir(exist_ok=False)
...
with (vmwork/'qemu.log').open('wb') as log:
 ...
 try:
  ...
  subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223',str(work/'package-input.tar'),'builder@127.0.0.1:/tmp/nia-root-extract.tar'],check=True,timeout=60)
  script='''set -eu
umask 022
...
sudo python3 tests/worker/check_root_extract.py --worker /usr/libexec/niaos/root-extract --target-base /run/nia-root-extract-test --report /tmp/nia-root-extract/extraction.json
sudo python3 tests/worker/check_root_bank.py --worker /usr/libexec/niaos/root-extract --base /run/nia-root-extract-test --report /tmp/nia-root-extract/bank.json
sudo python3 tests/worker/check_root_reinspection.py --worker /usr/libexec/niaos/root-extract --base /run/nia-root-extract-test --report /tmp/nia-root-extract/result.json
...
'''
  ...
  for name in ['extraction.json','bank.json','build-a.log','build-b.log','package-sha256.txt','install.log']:
