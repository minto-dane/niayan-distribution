...
work=Path('/evidence');vmwork=work/'vm-bootstrap-02';vmwork.mkdir(exist_ok=False)
...
cmp build-a/niaos-root-preparation_0.2.0_amd64.deb build-b/niaos-root-preparation_0.2.0_amd64.deb
cmp build-a/niaos-root-preparation-dbgsym_0.2.0_amd64.deb build-b/niaos-root-preparation-dbgsym_0.2.0_amd64.deb
sha256sum build-a/*.deb > package-sha256.txt
sudo dpkg -i dependencies/*.deb component.deb build-a/niaos-root-preparation_0.2.0_amd64.deb > install.log 2>&1
sudo dpkg-query -W niaos-pkgcore libgnat-14 niaos-root-preparation > installed-packages.txt
...
'''
  ...
  for report_name in ['result.json','installed-packages.txt','namespace.json','native-service.log','build-a.log','build-b.log','install.log','package-sha256.txt','build-a/niaos-root-preparation_0.2.0_amd64.buildinfo']:
  ...
  if r.returncode==0:
   subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/var/tmp/nia-service/build-a/niaos-root-preparation_0.2.0_amd64.deb',str(vmwork/'niaos-root-preparation_0.2.0_amd64.deb')],check=True,timeout=30)
