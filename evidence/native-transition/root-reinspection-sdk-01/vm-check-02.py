...
work=Path('/evidence');vmwork=work/'vm-sdk-02';vmwork.mkdir(exist_ok=False)
...
cmp native/root_bank.py /usr/libexec/niaos/root_bank.py
...
uname -a
stat -c '%a %u %g %n' /tmp/nia-root-extract/fixtures
findmnt -T /run/nia-root-extract-test -o TARGET,FSTYPE,OPTIONS
sudo python3 native/worker/check_root_preparation.py --reinspect --configured --worker /usr/libexec/niaos/root-extract --driver /tmp/nia-root-extract/build/run_root_configuration_tests --loader /tmp/nia-root-extract/lib/ld-linux-x86-64.so.2 --libraries /tmp/nia-root-extract/lib --fixtures /tmp/nia-root-extract/fixtures/conffiles --base /run/nia-root-extract-test --report /tmp/nia-root-extract/configured/result.json
sudo python3 native/worker/check_root_preparation.py --reinspect --worker /usr/libexec/niaos/root-extract --driver /tmp/nia-root-extract/build/run_root_archive_tests --loader /tmp/nia-root-extract/lib/ld-linux-x86-64.so.2 --libraries /tmp/nia-root-extract/lib --fixtures /tmp/nia-root-extract/fixtures/root-preparation --base /run/nia-root-extract-test --report /tmp/nia-root-extract/plain/result.json
...
'''
  ...
  for name in ['configured','plain','install.log']:
   subprocess.run(['/evidence/scp-wrapper','-r','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/tmp/nia-root-extract/'+name,str(vmwork/name)],check=False,timeout=30)
