...
static int exchange(const char *path, const char *packet, int length,
    const char *expected, int expected_length, uint64_t deadline, int archive_fd, int reservation_fd) {
...
int nia_root_prepare(const char *path, const char *generation, const char *root,
    ...
    return exchange(path, packet, length, expected, expected_length, deadline, archive_fd, reservation_fd);
}
