...
work=Path('/evidence');vmwork=work/'vm-credential-05';vmwork.mkdir(exist_ok=False)
...
with (vmwork/'qemu.log').open('wb') as log:
 ...
 try:
  ...
  while True:
   ...
   time.sleep(1)
  print('VM archive credential ready',flush=True)
  subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223',str(work/'runtime.tar'),'builder@127.0.0.1:/var/tmp/nia-credential.tar'],check=True,timeout=60)
  script="""set -eu
mkdir -m 755 /var/tmp/nia-credential
cd /var/tmp/nia-credential
tar -xf /var/tmp/nia-credential.tar
sudo dpkg -i dependencies/*.deb > install.log 2>&1
sudo chown -R root:root runtime
sudo dpkg-query -W python3-tuf python3-securesystemslib python3-cryptography systemd libgnat-14 > installed-packages.txt
sudo python3 runtime/distribution/native/worker/check_archive_credential.py --runtime /var/tmp/nia-credential/runtime --report /var/tmp/nia-credential/result.json
"""
  ...
  print('Archive credential VM exit',r.returncode,flush=True)
  for name in ['result.json','installed-packages.txt','install.log','accepted.log','wrong-public-key.log','short-credential.log','missing-credential.log']:
   subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/var/tmp/nia-credential/'+name,str(vmwork/name)],timeout=30)
