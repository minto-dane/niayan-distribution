...
with (work/'qemu.log').open('wb') as log:
    ...
    try:
        ...
        subprocess.run(scp+[str(lab/'runtime-normal.tar'),'builder@127.0.0.1:/home/builder/runtime.tar'],check=True,timeout=60)
        with (work/'guest.log').open('wb') as out:
            result=subprocess.run(ssh+['sh','-s'],input=(lab/'guest-normal.sh').read_bytes(),stdout=out,stderr=subprocess.STDOUT,timeout=420)
        ...
        for name in ['install.log','bootstrap.log','result.json']:
