...
(cd source && dpkg-buildpackage -us -uc) > build.log 2>&1
sudo dpkg -i dependencies/*.deb component.deb niaos-root-preparation_0.9.0_amd64.deb > install.log 2>&1
...
sudo python3 check_root_session_cancel.py --disposable-vm --report /home/builder/niayan-monitor/result.json
