...
subprocess.run(['qemu-img','create','-f','raw',str(work/'bank.raw'),'64M'],check=True)
...
args=['qemu-system-x86_64','-machine','q35','-accel','kvm','-cpu','host','-m','2048','-smp','1',
      ...
      '-drive','file='+str(work/'bank.raw')+',if=virtio,format=raw',
...
with (work/'qemu.log').open('wb') as log:
    ...
    try:
        ...
        subprocess.run(scp+[str(lab/'runtime.tar'),'builder@127.0.0.1:/home/builder/runtime.tar'],check=True,timeout=60)
        with (work/'guest.log').open('wb') as out:
            result=subprocess.run(ssh+['sh','-s'],input=(lab/'guest.sh').read_bytes(),stdout=out,stderr=subprocess.STDOUT,timeout=420)
        ...
        for name in ['build.log','install.log','bootstrap.log','result.json','*.deb','*.dsc','*.tar.xz','*.buildinfo','*.changes']:
