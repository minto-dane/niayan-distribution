...
package body Root_Archive_Stage_Test with SPARK_Mode => Off is
   ...
   procedure Run (Store : in out MC_Store.Store; Store_Path : String;
      ...
      begin
         ...
         if not Missing_FD then Root_FD := (if Wrong_Source and then Phase = "stage:advance-inputs" then MC_FS.Native (Stage_Root) else Source_FD); end if; Status := OK;
      end Observe;
   ...
   begin
      ...
      if Configured then
         ...
         MC_FS.Open_Root (Root_Path, Stage_Root, Status, Private_Only => True); Need ("different source root fixture");
         Wrong_Source := True;
         Stage.Advance (Root_Path, State_Path, Store_Path, Expected, Count, Deadline, Status);
         Expect (Status /= OK and then Count = 0, "provider success with different physical source is not current evidence");
         Wrong_Source := False; MC_FS.Close (Stage_Root);
