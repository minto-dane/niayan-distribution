...
work=Path('/evidence');vmwork=work/'vm-supply-planner-01';vmwork.mkdir(exist_ok=False)
...
sudo python3 runtime/distribution/native/worker/check_archive_observer.py --runtime /var/tmp/nia-observer/runtime --report /var/tmp/nia-observer/result.json --planning
"""
  ...
  for name in ['result.json','installed-packages.txt','install.log','accepted.log','planned.log','planned-again.log','planning-trust-change.log','planning-floor-missing.log','second-request.log','wrong-control.log','wrong-observer-uid.log','nonprivate-state.log','wrong-state-owner.log','root-peer.log','changed-original.log','unprotected-config.log','wrong-credential.log','untrusted-https.log','observer-journal.log']:
