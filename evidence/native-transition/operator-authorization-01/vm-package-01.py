...
with (vmwork/'qemu.log').open('wb') as log:
 ...
 try:
  ...
  subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223',str(work/'runtime.tar'),'builder@127.0.0.1:/home/builder/nia-operator-01.tar'],check=True,timeout=60)
  script='''set -eu
umask 022
mkdir -m 700 /home/builder/nia-operator-01
cd /home/builder/nia-operator-01
tar -xf /home/builder/nia-operator-01.tar
...
cmp build-a/niaos-root-preparation_0.7.0_amd64.deb build-b/niaos-root-preparation_0.7.0_amd64.deb
cmp build-a/niaos-root-preparation-dbgsym_0.7.0_amd64.deb build-b/niaos-root-preparation-dbgsym_0.7.0_amd64.deb
cmp build-a/niaos-root-preparation_0.7.0.dsc build-b/niaos-root-preparation_0.7.0.dsc
cmp build-a/niaos-root-preparation_0.7.0.tar.xz build-b/niaos-root-preparation_0.7.0.tar.xz
sha256sum build-a/*.deb build-a/*.dsc build-a/*.tar.xz > package-sha256.txt
sudo dpkg -i dependencies/*.deb build-a/niaos-root-preparation_0.7.0_amd64.deb > install.log 2>&1
sudo systemd-analyze verify niaos-root-preparation.service niaos-root-preparation.socket niaos-root-bank-check.service niaos-root-session.socket niaos-root-session.service niaos-root-session-seal.service var-lib-niaos-roots.mount
...
uname -a
sha256sum /usr/libexec/niaos/root-extract /usr/share/polkit-1/actions/org.niaos.package.policy
sudo python3 check_operator_authorization.py --disposable-vm --library /home/builder/nia-operator-01/operator-authorization.so --ada /home/builder/nia-operator-01/operator-ada --report /home/builder/nia-operator-01/result.json
...
'''
  ...
  with (vmwork/'service-journal.log').open('wb') as out:
   subprocess.run(ssh+['sudo journalctl -u polkit.service --no-pager -n 150'],stdout=out,stderr=subprocess.STDOUT,timeout=20)
  for name in ['result.json','result.ada.log','result.policy.txt','build-a.log','build-b.log','package-sha256.txt','install.log']:
   subprocess.run(['/evidence/scp-wrapper','-r','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-operator-01/'+name,str(vmwork/name)],check=False,timeout=30)
  (vmwork/'packages').mkdir()
  for pattern in ['*.deb','*.dsc','*.tar.xz','*.buildinfo','*.changes']:
   subprocess.run(['/evidence/scp-wrapper','-S','/evidence/ssh-wrapper',*options,'-P','22223','builder@127.0.0.1:/home/builder/nia-operator-01/build-a/'+pattern,str(vmwork/'packages')],check=True,timeout=30)
