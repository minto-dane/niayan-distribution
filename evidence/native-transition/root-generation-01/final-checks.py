...
os.umask(0o077)
root=Path('/workspace/pkgcore');out=Path('/evidence/final');out.mkdir();os.chdir(root)
checks=[]
def run(name,args):
...
def case(name,dirs):
 ...
 return p
run('build',['gprbuild','-j1','-P','tests.gpr','run_root_archive_tests.adb','run_generation_stage_tests.adb','run_generation_publication_tests.adb'])
