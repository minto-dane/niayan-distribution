...
def main():
    ...
    payloads = {'audit.py': HERE / 'audit.py', 'baseline.json': HERE / 'baseline.json',
                ...
                'niaos-hostctl-confinement.service': HERE / 'rootfs/usr/lib/systemd/system/niaos-hostctl-confinement.service',
...
install -m 0644 "$probe/nia-hostctl" /etc/apparmor.d/nia-hostctl
install -m 0644 "$probe/niaos-hostctl-confinement.service" /run/systemd/system/niaos-hostctl-confinement.service
systemd-analyze verify /run/systemd/system/niaos-hostctl-confinement.service
systemctl daemon-reload
systemctl start niaos-hostctl-confinement.service
systemctl is-active niaos-hostctl-confinement.service
/usr/libexec/nia/hostctl inspect > "$probe/hostctl.txt"
cat "$probe/hostctl.txt"
for feature in SYSTEMD_PID_ONE CGROUP_V2 MEMORY_CONTROLLER PID_CONTROLLER BPF_RESTRICTED KERNEL_POINTERS_RESTRICTED KERNEL_LOG_RESTRICTED FULL_ASLR; do
  grep -x "$feature=SATISFIED" "$probe/hostctl.txt"
done
