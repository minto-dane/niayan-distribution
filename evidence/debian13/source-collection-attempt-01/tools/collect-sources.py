...
def historical_download(target, name, version, binary=False):
    ...
    with tempfile.TemporaryDirectory(prefix='nia-debsnap-') as temporary:
        ...
        command = ['debsnap', '--verbose', '--destdir', str(download)]
        if binary:
            command += ['--architecture', 'amd64', '--architecture', 'all']
        subprocess.run([*command, name, version],
                       env={**os.environ, 'HOME': temporary}, timeout=900, check=True)
