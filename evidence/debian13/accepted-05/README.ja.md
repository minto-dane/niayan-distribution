...
SHA-256: `5b6c3038723966dc9ac60cd7b3e3b36388b405f2f865e446ab0546c378b2901c`。
ISO全体をホストへコピー後、同じSHA-256を再確認した。

## 実行した受入

[一括実行report](acceptance/report.json)の6項目が成功した。実行時間792.992秒。メモリ3 GiB・swapなし・CPU 1コア分・128プロセスの外側kernel制限を確認し、QEMUゲストはメモリ2 GiB・1 vCPUで必ず順番に実行した。
...
独自パッケージの再ビルドで58個の登録Adaテストmainを実行した。[19 DEBの比較](distro-deb-comparison-04.json)は全件一致し、デバッグパッケージも含む。[8組のnative source package](distro-native-source-comparison-04.json)の`.dsc`・source tarも16ファイルすべて一致した。識別DEBの導入・再導入・remove・purgeによるDebian識別情報の復元も成功した。別の試作時には同じ版のbase-files再導入も確認しているが、将来のbase-files版への更新試験とは扱わない。
...
7コンポーネントの配置とhost observer確認は、未接続のCapsule/native broker等の製品機能の完成を意味しない。05と06のISO全体の[再構築比較は不一致](../reproducibility-05-06/README.ja.md)だった。この旧ISO単独の対応ソース収集は未実施。後続の[ISO 09](../accepted-09/source-collection/README.ja.md)では収集・補完・コピー後の検査を完了した。公開先と署名運用は別の確認事項である。本番認定・GitHubへの公開は行っていない。
