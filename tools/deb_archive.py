...
def inspect_bytes(raw:bytes,*,architecture:str='amd64')->dict:
    if not isinstance(architecture,str) or not re.fullmatch(r'[a-z0-9][a-z0-9-]{0,31}',architecture) or architecture=='all':raise Invalid('invalid intake architecture')
    ...
    if info['architecture'] not in (architecture,'all'):raise Invalid('architecture outside intake policy')
